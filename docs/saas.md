...
- **Learning sessions** run server-side: one Bubble Tea program per WebSocket,
  rendered by xterm.js in the browser. Questions are still generated by the
  LLM with the child's learner profile — the core Mathiz experience, hosted.
