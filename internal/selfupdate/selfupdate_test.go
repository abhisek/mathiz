...
func TestCheck(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			body: `{
				"tag_name": "v1.1.0",
				"html_url": "https://github.com/abhisek/mathiz/releases/tag/v1.1.0"
			}`,
		...
		},
		{
			...
			body: `{
				"tag_name": "v1.0.0",
				"html_url": "https://github.com/abhisek/mathiz/releases/tag/v1.0.0"
			}`,
			currentVersion: "v1.0.0",
			wantAvailable:  false,
		},
...
func TestCheckTrailingSlashBaseURL(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/repos/abhisek/mathiz/releases/latest", r.URL.Path)
